...
ggtitle <- function (x) { return(ggplot2::ggtitle("")) }
  ...
  for(i in 1:nrow(res_sig)) {
    ...
    tryCatch({
      TCGAanalyze_survival_override(
        ...
        label=c(paste0(gene_symbol, " below median expression for SARIFA annotated TCGA-STAD set"), paste0(gene_symbol, " above median experssion for SARIFA annotated TCGA-STAD set"))
      )
    })
  }
}
...
plot_cluster_heatmap <- function(dds, res, path) {
  ...
  annotation_col_col_names <- c("SARIFA Group", 
                                "Molecular Subtype",
                                "Absolute Purity",
  ...
  pheatmap(
    ...
    # main="Gene expression heatmap of per gene z-transformed counts,\nfor significantly expressed genes with adjusted\np-value < 0.05 and abs(lfc) > 1.0, clustered\n by gene and by sample with clustering_method=complete,\n multiple annotations per sample",
    width=14,
    height=9
  )
  ...
  pheatmap(
    ...
    # main="Gene expression heatmap of per gene z-transformed counts,\nfor significantly expressed genes with adjusted\np-value < 0.05 and abs(lfc) > 1.0, clustered\n by gene and by sample with clustering_method=average,\n multiple annotations per sample",
    width=14,
    height=9
  )
  ...
  pheatmap(
    ...
    # main="Gene expression heatmap of per gene z-transformed counts,\nfor significantly expressed genes with adjusted\np-value < 0.05 and abs(lfc) > 1.0, clustered\n by gene and by sample with clustering_method=ward.D,\n multiple annotations per sample",
    width=14,
    height=9
  )
  ...
  pheatmap(
    ...
    # main="Gene expression heatmap of per gene z-transformed counts,\nfor significantly expressed genes with adjusted\np-value < 0.05 and abs(lfc) > 1.0, clustered\n by gene and by sample with clustering_method=ward.D2,\n multiple annotations per sample",
    width=14,
    height=9
  )
}
...
export_butchr_input <- function(dds, res, path) {
  ...
  # write.csv(data.matrix(as.data.frame(counts)), paste0(path, "export/butchr/matrix.csv"))
  # write.csv(as.data.frame(annotation_col), paste0(path, "export/butchr/annotation.csv"), row.names=FALSE)
}
...
plot_deseq_results <- function(dds, res, enrich_res, path) {
  ...
  #plot_enrich_results(enrich_res, path)
  return()
...
plot_export_compare_results <- function(res_reduced, res_strict, path) {
  res_merged <- merge(as.data.frame(res_reduced), as.data.frame(res_strict), by=0)
  ...
  res_reduced_sig <- subset(res_reduced_sig, abs(res_reduced_sig$log2FoldChange) > 1.0)
  
  res_strict_sig <- subset(res_strict, res_strict$padj < 0.05)
  res_strict_sig <- subset(res_strict_sig, abs(res_strict_sig$log2FoldChange) > 1.0)
